...
spy = pd.read_csv("/Users/hridhaan/Downloads/Spring_Weeks/Spring_Week/Projects/Quant_CV_Projects/Monte_Carlo_Project1/Data/^GSPC.csv",header=2)
...
spy.sort_values(by = "Date", ascending = True, inplace = True)
...
spy.drop_duplicates(subset = "Date", inplace = True)
...
spy.rename(columns = {"Adj Close": "S&P500"}, inplace = True)
...
combined3 = pd.merge(combined2, spy[["Date","S&P500"]], on = "Date")
...
combined3["S&P500 Daily Returns"] = combined3["S&P500"].pct_change()
...
combined3.dropna(subset = "S&P500 Daily Returns", inplace = True)
...
spy_meanR = combined3["S&P500 Daily Returns"].mean()
...
spy_stdR = combined3["S&P500 Daily Returns"].std()
...
spy_annualisedR = spy_meanR * 252
xom_annualisedR = xom_meanR * 252

jpm_annualisedV = jpm_stdR * np.sqrt(252)
msft_annualisedV = msft_stdR * np.sqrt(252)
spy_annualisedV = spy_stdR * np.sqrt(252)
xom_annualisedV = xom_stdR * np.sqrt(252)

# Create a new dataframe comapring individual assets
asset_summary = {
    "Annual Return": [jpm_annualisedR,msft_annualisedR,xom_annualisedR,spy_annualisedR],
    "Annual Volatility": [jpm_annualisedV,msft_annualisedV,xom_annualisedV,spy_annualisedV]
    }

summary_df = pd.DataFrame(asset_summary, index = ["JPM","MSFT","XOM","S&P500"])

# Calculating correlation matrix to see how individual assets returns move in relation to each other
correlation_matrix = combined3[["MSFT Daily Returns","JPM Daily Returns","XOM Daily Returns","S&P500 Daily Returns"]].corr()
...
portfolio1 = np.dot(weights_1,[msft_annualisedR,jpm_annualisedR,xom_annualisedR,spy_annualisedR]) 
portfolio2 = np.dot(weights_2,[msft_annualisedR,jpm_annualisedR,xom_annualisedR,spy_annualisedR]) 
portfolio3 = np.dot(weights_3,[msft_annualisedR,jpm_annualisedR,xom_annualisedR,spy_annualisedR]) 
portfolio4 = np.dot(weights_4,[msft_annualisedR,jpm_annualisedR,xom_annualisedR,spy_annualisedR]) 
portfolio5 = np.dot(weights_5,[msft_annualisedR,jpm_annualisedR,xom_annualisedR,spy_annualisedR]) 

# Calculating covariance matrix for portfolio variance formula
covariance_matrix = combined3[["MSFT Daily Returns","JPM Daily Returns","XOM Daily Returns","S&P500 Daily Returns"]].cov()
